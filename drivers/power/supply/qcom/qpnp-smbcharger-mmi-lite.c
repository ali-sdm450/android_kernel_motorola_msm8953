...
out:
        pr_smb_rt(PR_MISC, "CHGR_status = %d\n", status);
...
{
        pr_smb(PR_STATUS, "smbchg_charging_en1 %d\n", en? 1: 0);
        pr_smb(PR_STATUS, "charging_limit_modes %d\n", chip->charging_limit_modes);
        pr_smb(PR_STATUS, "enable_charging_limit %d\n", chip->enable_charging_limit ? 1 : 0);
        pr_smb(PR_STATUS, "is_factory_image %d\n", chip->is_factory_image ? 1 : 0);
	if ((chip->charging_limit_modes == CHARGING_LIMIT_RUN)
		&& (chip->enable_charging_limit)
		&& (chip->is_factory_image)) {
                pr_smb(PR_STATUS, "smbchg_charging_en change en = 0\n");
		en = 0;
        }
	if (en == true) {
        ...
        pr_smb(PR_STATUS, "smbchg_charging_en2 %d\n", en? 1: 0);
...
static struct power_supply *get_parallel_psy(struct smbchg_chip *chip)
{
	...
	chip->parallel.psy = NULL/*power_supply_get_by_name("usb-parallel")*/;//TBD
	if (!chip->parallel.psy)
...
static int smbchg_parallel_usb_charging_en(struct smbchg_chip *chip, bool en)
{
	...
	pval.intval = en;
        pr_smb(PR_STATUS, "smbchg_parallel_usb_charging_en, enabled = %s\n", en ? "enabled" : "disabled");
	return power_supply_set_property(parallel_psy,
...
static void smbchg_parallel_usb_disable(struct smbchg_chip *chip)
{
	...
	smbchg_set_fastchg_current(chip, current_max_ma);
        pr_smb(PR_STATUS, "smbchg_parallel_usb_disable, usb_target_current_ma =%d\n", chip->usb_target_current_ma);
	chip->usb_tl_current_ma =
		calc_thermal_limited_current(chip, chip->usb_target_current_ma);
        pr_smb(PR_STATUS, "smbchg_parallel_usb_disable, usb_tl_current_ma =%d\n", chip->usb_tl_current_ma);
        pr_smb(PR_STATUS, "smbchg_parallel_usb_disable\n");
...
static void smbchg_parallel_usb_enable(struct smbchg_chip *chip)
{
	...
	new_parallel_cl_ma = total_current_ma / 2;
        /*new_parallel_cl_ma = total_current_ma
			* (100 - smbchg_main_chg_icl_percent) / 100;*///TBD
							...
							int current_ma)
{
	...
	chip->usb_tl_current_ma =
        ...
        pr_smb(PR_STATUS, "smbchg_set_thermal_limited_usb_current_max\n");
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		chip->chg_enabled = val->intval;
                pr_smb(PR_STATUS, "chg_enabled = %s\n", chip->chg_enabled ? "enabled" : "disabled");
	...
	if (chip->usb_psy &&
		...
		if ((prop.intval == POWER_SUPPLY_TYPE_USB_CDP) ||
			...
			chip->charger_rate = POWER_SUPPLY_CHARGE_RATE_TURBO;
			 ...
			 /*chip->hvdcp_det_done && */chip->aicl_complete &&//TBD
...
static int smbchg_is_ta_charger(struct power_supply *psy, int current_limit)
{
	...
	//rc = chip->usb_psy->get_property(chip->usb_psy,
        ...
        rc = power_supply_get_property(chip->usb_psy, POWER_SUPPLY_PROP_TYPE, &prop);//TBD
	if (rc < 0) {
...
static void smbchg_external_power_changed(struct power_supply *psy)
{
	...
	rc = power_supply_get_property(chip->usb_psy, POWER_SUPPLY_PROP_CURRENT_MAX,
			&pval);
	if (rc < 0)
		dev_err(chip->dev,
			"could not read USB current_max property, rc=%d\n", rc);
	else
		current_limit = pval.intval / 1000/*500*/;
						...
						enum power_supply_type type)
{
	...
	if (rc < 0) {
        ...
        pr_smb(PR_STATUS, "chip->usb_current_max %d\n",
	...
	if (type == POWER_SUPPLY_TYPE_UNKNOWN) {
		...
		if (rc < 0)
			pr_err("Couldn't vote for new USB ICL rc=%d\n", rc);*/
                ...
                pr_smb(PR_STATUS, "chip->usb_current_max set 0: %d\n",
		chip->usb_current_max);
		chip->usb_supply_type = type;
	}
	/*
	...
	if ((reg & hvdcp_sel) && is_usb_present(chip)) {
		...
		if (is_usb_present(chip)) {
			//power_supply_set_supply_type(chip->usb_psy,
					//POWER_SUPPLY_TYPE_USB_HVDCP);//TBD
                        //chip->usb_supply_type = POWER_SUPPLY_TYPE_USB_HVDCP;
...
static int handle_dc_removal(struct smbchg_chip *chip)
{
	if (chip->dc_psy_type != -EINVAL)
                //power_supply_set_online(&chip->dc_psy, chip->dc_present);//TBD
		power_supply_changed(chip->dc_psy);	
...
static void usb_insertion_work(struct work_struct *work)
{
	...
	/*rc = chip->usb_psy->get_property(chip->usb_psy,
			POWER_SUPPLY_PROP_CURRENT_MAX, &prop);*///TBD
        rc = power_supply_get_property(chip->usb_psy,
			POWER_SUPPLY_PROP_CURRENT_MAX, &prop);
	if (rc < 0)
		dev_err(chip->dev,
	...
	if (chip->usb_psy) {
		...
		if ((chip->supply_type != POWER_SUPPLY_TYPE_USB) &&
			...
			//power_supply_set_chg_present(chip->usb_psy,
					//chip->usb_present);//TBD moto add gerrit 871385
		...
		//power_supply_set_dp_dm(chip->usb_psy,
				//POWER_SUPPLY_DP_DM_DPR_DMR);//TBD
                smbchg_request_dpdm(chip, false);
		schedule_work(&chip->usb_set_online_work);
		//rc = power_supply_set_health_state(chip->usb_psy,
				//POWER_SUPPLY_HEALTH_UNKNOWN);//TBD
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
	...
	if (chip->usb_psy) {
		pr_smb(PR_MISC, "setting usb psy type = %d\n",
				chip->supply_type);
		//power_supply_set_supply_type(chip->usb_psy, chip->supply_type);//TBD
		...
		if ((chip->supply_type != POWER_SUPPLY_TYPE_USB) &&
			...
			//power_supply_set_chg_present(chip->usb_psy,
		        ...
		        extcon_set_cable_state_(chip->extcon, EXTCON_USB,
				        chip->usb_present);//TBD MOT ?? chg present

		} else {
			pr_smb(PR_MISC, "Set usb present to %d\n",
					chip->usb_present);
			//power_supply_set_present(chip->usb_psy,
					//chip->usb_present);//TBD
                        extcon_set_cable_state_(chip->extcon, EXTCON_USB,
				chip->usb_present);
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
	case POWER_SUPPLY_PROP_SDP_CURRENT_MAX:
                pr_smb(PR_STATUS, "set current %d\n", val->intval / 1000);
...
static int weak_charger_check(void *_chip)
{
	...
	if (chip->usb_psy) {
		/*rc = chip->usb_psy->get_property(chip->usb_psy,
			POWER_SUPPLY_PROP_TYPE, &prop);*///TBD
                rc = power_supply_get_property(chip->usb_psy, POWER_SUPPLY_PROP_TYPE, &prop);//TBD
		if (rc < 0) {
			dev_err(chip->dev,
			"could not read USB current_max property, rc=%d\n", rc);
		...
		if (1 == chip->weak_charger_valid_cnt) {
			...
			smbchg_set_usb_current_max(
				chip, chip->usb_tl_current_ma);
	                pval.intval = chip->usb_target_current_ma * 1000;
	                power_supply_set_property(chip->usb_psy, POWER_SUPPLY_PROP_CURRENT_MAX,
					&pval);
			/*power_supply_set_current_limit(chip->usb_psy,
				chip->usb_target_current_ma * 1000);*///TBD
			}
		} else {
		chip->adapter_vbus_collapse_flag = false;
		chip->weak_charger_valid_cnt = 0;
...
static irqreturn_t usbin_uv_handler(int irq, void *_chip)
{
	...
	if (reg && chip->usb_psy && !/*chip->usb_psy->get_property(chip->usb_psy,
						  	POWER_SUPPLY_PROP_TYPE,
							&prop)*/
                      power_supply_get_property(chip->usb_psy, POWER_SUPPLY_PROP_TYPE, &prop)) {//TBD
...
static int smbchg_force_apsd(struct smbchg_chip *chip)
{
	...
	//power_supply_set_chg_present(chip->usb_psy, 0);
        extcon_set_cable_state_(chip->extcon, EXTCON_USB, 0);//TBD, moto added
	//power_supply_set_present(chip->usb_psy, 0);
        extcon_set_cable_state_(chip->extcon, EXTCON_USB, 0);
	//power_supply_set_online(chip->usb_psy, 0);//TBD
	chip->usb_online = 0;
	power_supply_changed(chip->usb_psy);
	//power_supply_set_dp_dm(chip->usb_psy,
			       //POWER_SUPPLY_DP_DM_DPR_DMR);//TBD
	...
	for_each_available_child_of_node(chip->pdev->dev.of_node, child) {
		...
		case SMBCHG_LITE_MISC_SUBTYPE:
			...
			if (rc < 0)
				return rc;
			rc = smbchg_request_irq(chip, child,
				&chip->safety_timeout_irq, "wdog-timeout",
				safety_timeout_handler, flags);
			if (rc < 0)
				return rc;
		...
		int batt_soc)
{
	enum charging_limit_modes charging_limit_modes
						= chip->charging_limit_modes;
        pr_info("charging_limit_modes, chip->chg_li = %d\n", chip->charging_limit_modes);

	if ((charging_limit_modes != CHARGING_LIMIT_RUN)
		&& (batt_soc >= chip->upper_limit_capacity)) {
                pr_info("charging_limit_modes, change to limit run, batt_soc %d, chip->upper_limit_capacity %d\n", batt_soc, chip->upper_limit_capacity);
		charging_limit_modes = CHARGING_LIMIT_RUN;
	} else if ((charging_limit_modes != CHARGING_LIMIT_OFF)
			&& (batt_soc <= chip->lower_limit_capacity)) {
                pr_info("charging_limit_modes, change to limit off, batt_soc %d, chip->lower_limit_capacity %d\n", batt_soc, chip->lower_limit_capacity);
	...
	if (charging_limit_modes != chip->charging_limit_modes) {
		chip->charging_limit_modes = charging_limit_modes;
                pr_info("final charging_limit_modes %d\n", charging_limit_modes);
		if (charging_limit_modes == CHARGING_LIMIT_RUN)
			smbchg_charging_en(chip, 0);
		else
			smbchg_charging_en(chip, 1);
...
static void smbchg_heartbeat_work(struct work_struct *work)
{
	...
	int index;
        ...
        pr_info("smbchg_heartbeat_work\n");
	smbchg_stay_awake(chip, PM_HEARTBEAT);
	if (smbchg_check_and_kick_aicl(chip)) {
                pr_info("smbchg_check_and_kick_aicl return true\n");
		goto end_hb;
        }
	set_property_on_fg(chip, POWER_SUPPLY_PROP_UPDATE_NOW, 1);
			 ...
			 unsigned long event, void *unused)
{
	...
	case SYS_POWER_OFF:
		...
		//power_supply_set_present(chip->usb_psy, 0);
                extcon_set_cable_state_(chip->extcon, EXTCON_USB, 0);
		//power_supply_set_chg_present(chip->usb_psy, 0);
                extcon_set_cable_state_(chip->extcon, EXTCON_USB, 0);//TBD
		//power_supply_set_online(chip->usb_psy, 0);//TBD
		chip->usb_online = 0;
		power_supply_changed(chip->usb_psy);
		if (!chip->factory_mode)
